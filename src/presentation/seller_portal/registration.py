...
async def _show_onboarding_faq(event: Message | CallbackQuery, state: FSMContext, ui: MessageManager):
    ...
    line = "▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰"
    text = (
        f"📖 <b>ОСНОВЫ РАБОТЫ // GDPX ACADEMY</b>\n"
        f"{line}\n"
        f"❖ <b>Академия GDPX:</b>\n"
        f"Конкуренция в прошлом. Время монополии.\n"
        f" Мы даем знания, вы производите актив, мы его покупаем.\n\n"
        f"- Ты в системе. Это закрытая платформа для тех, кто поставляет eSIM. \n"
        f"Здесь нет лишней суеты: ты даешь объем - мы платим.\n\n"
        f"<b>КАК ЭТО РАБОТАЕТ:</b>\n\n"
        f"▫️ <b>ЗАГРУЗКА:</b> Грузи eSIM пачками до 50 штук за раз. Бот работает мгновенно. Принимаем только четкие фото и читаемые сканы.\n\n"
        f"▫️ <b>ПРОВЕРКА:</b> Модераторы работают в порядке очереди. Обычно это занимает от 5 до 30 минут. Вердикт прост: либо зачёт, либо брак.\n\n"
        f"▫️ <b>ВЫПЛАТЫ:</b> Средства поступают после проверки всех позиций. Выплачиваем чеком CryptoBot (USDT) прямо здесь, в боте.\n\n"
        f"▫️ <b>КАРЬЕРА:</b> Твой доход растет с опытом. Ранги (от «Стажера» до «Акционера») дают автоматический бонус от +1% до +6% к каждой выплате.\n\n"
        f"▫️ <b>ПОДДЕРЖКА:</b> Есть вопрос - открывай тикет. Один запрос по делу, и администратор ответит в режиме реального времени.\n"
        f"{line}\n"
        f"Нажми «ДАЛЕЕ», чтобы пройти легализацию."
    )
...
async def _show_onboarding_codex(event: Message | CallbackQuery, state: FSMContext, ui: MessageManager):
    ...
    line = "▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰"
    chat_url = "https://t.me/+8uIOkwKFgjJhNjRi"

    text = (
        f"🛡 <b>ЛЕГАЛИЗАЦИЯ // КОДЕКС СИНДИКАТА</b>\n"
        f"{line}\n"
        f"Прежде чем начать, ты должен принять наши правила. \n"
        f"В «Академии» всё строится на дисциплине. Нарушил - вылетел.\n\n"
        f"<b>ПРАВИЛА КОДЕКСА:</b>\n\n"
        f"1. <b>КАЧЕСТВО - ПРИОРИТЕТ.</b> Нам нужен только рабочий материал. Загрузка мусора или неликвида карается мгновенным баном.\n\n"
        f"2. <b>СКОРОСТЬ - ТВОЙ ДОХОД.</b> Клиенты забирают товар быстро. Кто первый отгрузил, тот и заработал.\n\n"
        f"3. <b>ЧЕСТНОСТЬ - ОСНОВА.</b> Любая попытка обмануть систему, подсунуть дубли или фейки видна сразу. Наказание - BLACKLIST навсегда.\n\n"
        f"⚠️ <b>ПОСЛЕДНИЙ ШАГ:</b>\n"
        f"Чтобы терминал открыл доступ, ты обязан быть подписан на наш закрытый чат. Там вся актуальная информация по ценам и заказам.\n\n"
        f"🔗 <b>ВСТУПИТЬ:</b> Чат Академии GDPX ({chat_url})\n\n"
        f"{line}\n"
        f"Готов работать по правилам Синдиката?"
    )    
    kb = (PremiumBuilder()
