...
class AdminStatsService:
    ...
    async def get_owner_summary_stats(self) -> dict:
        ...
        sub_stats = await self._session.execute(
            select(
                func.count(func.distinct(case(
                    ((Submission.reviewed_at >= today_start) & (not Submission.is_archived), Submission.admin_id),
                    else_=None
                ))),
                func.sum(case(
                    ((Submission.status == SubmissionStatus.ACCEPTED) & (Submission.reviewed_at >= today_start) & (not Submission.is_archived), Submission.accepted_amount),
                    else_=0
                )),
                func.count(case(
                    ((Submission.status == SubmissionStatus.PENDING) & (not Submission.is_archived), Submission.id),
                    else_=None
                ))
            )
        )
        ...
        top_operator = await self._session.scalar(
            ...
            .where(Submission.reviewed_at >= today_start, not Submission.is_archived)
    ...
    async def get_detailed_finance_audit(self) -> dict:
        ...
        vol30 = await self._session.scalar(
            ...
            .where(Submission.status == SubmissionStatus.ACCEPTED, Submission.reviewed_at >= d30, not Submission.is_archived)
        )
    ...
    async def get_platform_stats(self, start: datetime, end: datetime) -> dict:
        ...
        ).where(Submission.created_at >= eff, Submission.created_at <= end, not Submission.is_archived)
    ...
    async def get_moderators_performance(self, start: datetime, end: datetime) -> list[dict]:
        ...
        stmt = (
            ...
            .where(Submission.reviewed_at >= eff, Submission.reviewed_at <= end, not Submission.is_archived)
    ...
    async def daily_sim_stats_for_month(self, year: int, month: int) -> list[dict[str, int | date]]:
        ...
        ).where(not Submission.is_archived, or_(and_(Submission.created_at >= eff, Submission.created_at < end), and_(Submission.reviewed_at >= eff, Submission.reviewed_at < end))
    ...
    async def get_recent_moderation_actions(self, limit: int = 10) -> list[dict]:
        stmt = select(ReviewAction, User.username, Submission.phone_normalized, Submission.id).join(User, ReviewAction.admin_id == User.id).join(Submission, ReviewAction.submission_id == Submission.id).where(not Submission.is_archived).order_by(desc(ReviewAction.created_at)).limit(limit)
    ...
    async def get_leaderboard(self, period: str = "all", page: int = 0, page_size: int = 15) -> tuple[list[dict], int]:
        _now, d30 = datetime.now(timezone.utc), datetime.now(timezone.utc) - timedelta(days=30)
        hidden = ["push", "fierroIT", "GDPX1", "GDPX0", "dyachx"]
        base_filters = [Submission.status == SubmissionStatus.ACCEPTED, not Submission.is_archived, ~User.username.in_(hidden), ~User.pseudonym.in_(hidden), ~User.nickname.in_(hidden)]
    ...
    @cached(ttl=60, key_prefix="user_rank")
    async def get_user_rank_info(self, telegram_id: int, period: str = "all") -> dict | None:
        """Получает ранг и объем конкретного пользователя."""
        _now, d30 = datetime.now(timezone.utc), datetime.now(timezone.utc) - timedelta(days=30)
        base_filters = [Submission.status == SubmissionStatus.ACCEPTED, not Submission.is_archived]
        if period == "30d":
            base_filters.append(Submission.reviewed_at >= d30)

        # 1. Находим объем текущего юзера
        user_stats_stmt = (
            select(func.sum(Submission.accepted_amount).label("e"), func.count(Submission.id).label("c"))
            .join(User, Submission.user_id == User.id)
            .where(User.telegram_id == telegram_id, *base_filters)
        )
        res = await self._session.execute(user_stats_stmt)
        user_row = res.one_or_none()
        if not user_row or user_row.e is None:
            return {"rank": "N/A", "earned": 0, "count": 0}

        user_earned = user_row.e

        # 2. Считаем сколько людей имеют объем больше
        rank_stmt = (
            select(func.count(func.distinct(User.id)))
            .join(Submission, Submission.user_id == User.id)
            .where(*base_filters)
            .having(func.sum(Submission.accepted_amount) > user_earned)
        )
        # SQLAlchemy count(distinct) с having сложнее, проще сделать подзапрос
        subq = (
            select(User.id)
            .join(Submission, Submission.user_id == User.id)
            .where(*base_filters)
        ...
        higher_count = await self._session.scalar(select(func.count()).select_from(subq)) or 0
        
        return {
            "rank": higher_count + 1,
            "earned": Decimal(user_earned).quantize(Decimal("0.01")),
            "count": user_row.c
        }
