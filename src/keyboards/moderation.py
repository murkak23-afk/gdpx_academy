...
from src.callbacks.moderation import AdminBatchCD, AdminGradeCD, AdminQueueCD, AdminSearchCD, AdminSellerQueueCD
...
def get_search_filters_kb(query: str, current_filter: str) -> InlineKeyboardMarkup:
    ...
    filters = [("📦 ВСЕ", "all"), ("🏮 ПРИО", "prio"), ("⚡️ SLA8", "sla8"), ("⌛️ SLA15", "sla15")]
...
def get_mod_dashboard_kb(total_pending: int, my_in_work: int) -> InlineKeyboardMarkup:
    ...
    builder.adjust(1)
    builder.back("mod_exit", "ВЕРНУТЬСЯ В МЕНЮ")
    return builder.as_markup()
...
def get_undo_kb(item_id: int) -> InlineKeyboardMarkup:
    ...
    return (
        PremiumBuilder().button("↩️ ОТМЕНИТЬ (UNDO)", AdminGradeCD(item_id=item_id, action="undo")).adjust(1).as_markup()
    )
